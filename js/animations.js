...
class AnimationManager {
  ...
  setupIntersectionObserver() {
    ...
    const animateElements = document.querySelectorAll(
      '.project-card, .skill-category, .testimonial, .social-grid a, .hero-content, .hero-image, .strength-card'
    );

    if (prefersReducedMotion) {
      // Skip animation entirely — just show everything
      animateElements.forEach(el => el.classList.add('animate-in'));
      return;
    }

    // Fallback: ensure all elements become visible after 800ms
    // in case IntersectionObserver never fires (e.g. already in viewport)
